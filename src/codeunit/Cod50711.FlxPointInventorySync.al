codeunit 50711 "FlxPoint Inventory Sync"
{
    ...
    begin
        ...
        while HasMorePages do begin
            Session.LogMessage('FlxPoint-InvSync-0006', 'Processing Inventory Page', Verbosity::Normal, DataClassification::SystemMetadata, TelemetryScope::ExtensionPublisher, 'PageNumber', Format(Page));
            // Clear and set up request for each page
            ...
            Session.LogMessage('FlxPoint-InvSync-0011', 'Inventory Variants Retrieved from Page', Verbosity::Normal, DataClassification::SystemMetadata, TelemetryScope::ExtensionPublisher, 'PageNumber', Format(Page), 'VariantCount', Format(JsonArray.Count));
            // Process each inventory variant
        ...
        if FlxPointInventory.FindSet() then begin
            ...
            repeat
                ...
                if (FlxPointInventory."Business Central QOH" <> FlxPointInventory.Quantity) or (FlxPointInventory."Business Central Price" <> FlxPointInventory.MSRP) or (FlxPointInventory."Inventory List Price" <> FlxPointInventory."Business Central Price") then begin
                    ...
                    JsonObject.Add('quantity', FlxPointInventory."Business Central QOH");
            ...
            // Log summary of the sync
            Session.LogMessage('FlxPointInventorySync', StrSubstNo('Inventory sync completed - Total variants checked: %1, Variants updated: %2', TotalVariants, UpdatedVariants), Verbosity::Normal, DataClassification::SystemMetadata, TelemetryScope::ExtensionPublisher, 'Category', 'FlxPoint');
        end;
    end;
    ...
    begin
        // Save JSON to blob for inspection
        Clear(TempBlob);
    ...
    begin
        ...
        if JobQueueLogEntry.FindLast() then
            EntryNo := JobQueueLogEntry."Entry No." + 1
        else
            EntryNo := 1;
        ...
        if IsError then
            Session.LogMessage('FlxPointInventorySync', Description + ': ' + ErrorMessage, Verbosity::Error, DataClassification::CustomerContent, TelemetryScope::ExtensionPublisher, 'Category', 'FlxPoint')
        else
