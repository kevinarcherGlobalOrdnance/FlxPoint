codeunit 50705 "FlxPoint Create Sales Order"
{
    ...
    begin
        ...
        if FlxPointFulfillmentReq.FindSet()then repeat if CreateSalesOrder(FlxPointFulfillmentReq)then begin
        ...
        if ErrorCount > 0 then Error(StrSubstNo('Processed %1 fulfillment requests with %2 errors.', TotalProcessed, ErrorCount));
    end;
    local procedure CreateSalesOrder(FulfillmentRequest: Record "FlxPoint Fulfillment Req"): Boolean var
    ...
    local procedure TryCreateSalesOrder(FulfillmentRequest: Record "FlxPoint Fulfillment Req"; var SalesOrderNo: Code[20]): Boolean var
        FlxPointFulfillmentReqLine: Record "FlxPoint Fulfillment Req Line";
        Customer: Record Customer;
        Item: Record Item;
        OrderIDText: Text[35];
    ...
    begin
        ...
        if FlxPointFulfillmentReqLine.FindSet()then repeat if FindItemBySKU(FlxPointFulfillmentReqLine.SKU, Item)then begin
                    SalesLine.Init();
    ...
    local procedure FindOrCreateCustomer(FulfillmentRequest: Record "FlxPoint Fulfillment Req"; var Customer: Record Customer): Boolean var
    ...
    local procedure FindItemBySKU(SKU: Text[50]; var Item: Record Item): Boolean var
